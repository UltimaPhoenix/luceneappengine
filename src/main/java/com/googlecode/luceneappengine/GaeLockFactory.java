	...
	public Lock makeLock(final String lockName) {
		return new Lock() {
			...
			public boolean obtain() throws IOException {
				...
				return objectify.transactNew(new Work<Boolean>() {
					@Override
					public Boolean run() {
						final boolean obtained;
						GaeLock gaeLock = objectify.load().key(Key.create(indexKey, GaeLock.class, lockName)).now();
						if(gaeLock == null) {
						...
						if(gaeLock.locked) {
							...
							gaeLock.locked = true;
							objectify.save().entity(gaeLock).now();
							obtained = true;
						}
						return obtained;
					}
